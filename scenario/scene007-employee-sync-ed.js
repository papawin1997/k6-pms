...
const bodySchema = JSON.parse(open('../data/body/employee_sync_body.json'));

export default function () {
  // Define
  const userIndex = ((__VU - 1) % 8);
  const baseUrl = "https://pmsapiuat.thaibev.com";
  const url = `${baseUrl}/admin/employee-sync-ed`
  ...
  // restore
  employeeSyncClear(baseUrl, header[userIndex]);
}

function employeeSyncClear(baseUrl, header) {
  const url = `${baseUrl}/admin/employee-sync-ed/clear`;
