...
const pmFormIDs = ["94624", "94625", "94626", "94627", "94628", "94629"];
...
export default function () {
  ...
  console.log(pmFormIDs[userIndex], stepNumbers[userIndex]);
  // Validate
  ...
  if (status == false) {
    ...
    if (stepNumbers[userIndex] <= 4) {
      sendBackForm(
        baseUrl,
        Number(pmFormIDs[userIndex]),
        stepNumbers[userIndex]
      );
    }
  }
}
...
function sendBackForm(baseUrl, pmFormID, stepNumber) {
  const bodySendBack = {
    pmFormID: pmFormID,
  ...
  const params = { headers: headerAdmin };
  const response = http.post(url, JSON.stringify(bodySendBack), params);
  // Validate
  const bodyResponse = response.json();
  const status = check(response, {
    "status is 200": (r) => r.status === 200,
  ...
  if (status == false) {
    const bodyString = JSON.stringify(bodyResponse);
    console.log(
      `send-back-performance, pm form id ${pmFormID} step number ${stepNumber}: ${bodyString} ,"bodySendBack": ${JSON.stringify(
        bodySendBack
      )}`
    );
  }
